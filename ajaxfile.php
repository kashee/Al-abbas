...
//
	...
	if(	isset($_REQUEST['centerid']) && $_REQUEST['centerid'] != '' && 
	...
	$uploadfile = FILE_URL.'centers/'.$imagename;

	move_uploaded_file($_FILES['cimage']['tmp_name'], $uploadfile);			


	if($id == 0){
		$iname = isset($_FILES['cimage']['name']) && $_FILES['cimage']['name'] != ''? $imagename : "";
		$query = "INSERT INTO `centers`(`name`,`type`,`logo`,`address`,`lat`,`lng`,`web_url`)
		 VALUES ('".$name."','".$type."','".$iname."','".$address."','".$lat."','".$lng."','".$web_url."')";
	}else{
		$imgQuery = isset($_FILES['cimage']['name']) && $_FILES['cimage']['name'] != ''? ",logo='".$imagename."'" : "";

		$query = "update centers set name='".$name."'"
		.",type='".$type."'" .",lat='".$lat."'" .",lng='".$lng."'" .",address='".$address."'" .",web_url='".$web_url."'"
		.$imgQuery				
		." where id = ".$id;
	}
	...
	if(isset($_REQUEST['product_id']) && $_REQUEST['product_id'] != '' && isset($_REQUEST['status']) && $_REQUEST['status'] != ''){
		$product_id = $_REQUEST['product_id'];
		$status = $_REQUEST['status'];
		$user_id = $_REQUEST['user_id'];
		$query = "update products set status = '". $status ." ' where id = ".(int)$product_id;
		mysqli_query($conn, $query);

		$get_usertokan = mysqli_query($conn,"SELECT * FROM ksmic_user where id='".$user_id."'");
		$resulttokan = mysqli_fetch_array($get_usertokan);

		

		if($status == 'Approved'){
			$message = 'Your listing request has been approved.';
			$title = 'Hooray!';
			$title2= 'Your listing request has been approved.';
		}else if($status == 'Blocked' || $status == 'Pending'){
			$message = 'Your listing request has been disapproved. Please follow our community guidelines and resubmit it again.';
			$title = 'Oops!';
			$title2= 'Your listing request has been disapproved. Please follow our community guidelines and resubmit it again.';
		}
		...
		$dArray = [
			...
			'image'  => FILE_URL.'userimg/'.$loginUser['userprofile'],
			'action' => 'ProductAdded'
		];		

		if(isset($resulttokan['usertokan'])){	
			$token[] =  $resulttokan['usertokan'];
			NotificationTopicSend($token, $title, $title2, 'ProductAdded',$dArray);
		}
			date_timezone_set('Europe/London');
			$created_at = date('Y-m-d h:i:s');
			mysqli_query($conn, "INSERT INTO `notifications` (`type`,`data`,`receiver_id`,`created_at`) VALUES ('ProductAdded','".json_encode($dArray)."','".$user_id."','".$created_at."')");
		
		
		echo "true";
	}

	if(isset($_REQUEST['userid']) && $_REQUEST['userid'] != '' && isset($_REQUEST['status']) && $_REQUEST['status'] != '' && isset($_REQUEST['email']) && $_REQUEST['email'] != ''){
		mysqli_query($conn,"update ksmic_user set `status`='".$_REQUEST['status']."' where id=".(int)$_REQUEST['userid']);
		if($_REQUEST['status'] == 0){
			$mail_message = 'Salam,
                               Your information is not correct.
                               Duas,
                               KSIMC Birmingham';
			send_mail($_REQUEST['email'], 'Admin disapprove your account.', $mail_message);
		}
		
		if($_REQUEST['status'] == 1){
			$mail_message = 'Salam,
                               Please check your home screen on Al Abbas App and see the login button.
                               Duas,
                               KSIMC Birmingham';
			send_mail($_REQUEST['email'], 'Admin approve your account.', $mail_message);
		}
	}
...
if(isset($_REQUEST['postid']) && $_REQUEST['postid'] != '' && isset($_REQUEST['statuspost']) && $_REQUEST['statuspost'] != ''){
	...
	if($_REQUEST['statuspost'] == 0){ 
		$title = 'Post Update';
		$msg = urldecode($resulttokan['title']).' post is unpublished.';
	}
	if($_REQUEST['statuspost'] == 1){
		$title = 'Post Update';
		$msg = urldecode($resulttokan['title']).' post is approved.';
		if(isset($resulttokan['usertokan'])){				
		push_notification_for_tokens($title,$msg,$resulttokan['usertokan']);
		}
		$title = urldecode($resulttokan['title']);
		$msg = 'Post is published on newsfeed.';
...
if(	isset($_REQUEST['commnunityid']) && $_REQUEST['commnunityid'] != '' && 
	...
	$query = "update community set community_name='".$community_name."'"
