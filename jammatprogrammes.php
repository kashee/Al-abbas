...
<title>Jammat Programme | Admin</title>
...
<div id="right-panel" class="right-panel"> 
      ...
      <div class="page-header float-right">
        <div class="page-title">
          <ol class="breadcrumb text-right">
            ...
            <li class="active">Jammat Programme</li>
          </ol>
        </div>
      </div>
    </div>
  ...
  <div class="container">
   ...
   if (isset($_POST["Submit"])) {
	...
	if(isset($_FILES['image']['name']))
	{
	    ...
	    $uploadfile = FILE_URL.'document/'.$imagename;
		...
		$sql_get = "Select * from ksmic_jamatprogdoc where id = 1";
		$res_get = mysqli_query($conn,$sql_get) or die(mysqli_error($conn));
		$result_get = mysqli_fetch_assoc($res_get);
		$pdoc = FILE_URL.'document/'.$result_get['document'];
		unlink($pdoc);
		
		$sql_insert = "Update ksmic_jamatprogdoc set document = '".$doc."' where id = 1";
		$res_insert = mysqli_query($conn,$sql_insert) or die(mysqli_error($conn));
		if($res_insert)
		{
			?>
				<div class="alert alert-info">Jamat Programme has been added successfully.</div>
			<?php 
		}
		else
		{
	...
	{
		?>
				<div class="alert alert-danger">Please select the file to upload.</div>
			<?php 
	}
  ...
  <div class="form-group row">
     <div class="col-sm-2">Upload Doc<small></small>:</div>
     <div class="col-sm-10">
      <input type="file" class="form-control" name="image"  style="width:70%" required>
    </div>
  </div>
